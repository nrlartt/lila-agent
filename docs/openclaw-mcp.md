...
- **`trustline entry is missing for account`:** the payer must have a **USDC trustline** + **USDC balance** on testnet. **Automatic (testnet):** set **`LILA_AUTO_SETUP_TESTNET_PAYER=true`** with **`STELLAR_NETWORK=stellar:testnet`** — MCP startup runs **Friendbot** (XLM) + **changeTrust** for USDC. You still add **USDC** manually ([Circle testnet faucet](https://faucet.circle.com)). **Manual:** `LILA_PAYER_SECRET=S... npm run payer:trustline` (same on-chain steps; no Friendbot if account already exists).
