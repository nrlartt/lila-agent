...
Tools: `lila_services` (merges **`mcpClient`** wallet setup hints), `lila_health`, `lila_payer_status` (alias `lila_mcp_payer_status`), `lila_query` (see [API](api.md)).

### Troubleshooting payer wallet

- **OpenClaw env must win over repo `.env`:** the MCP server loads `.env` with **`override: false`**, so **`LILA_AUTO_CREATE_PAYER_WALLET`** and **`LILA_PAYER_SECRET`** set in **`mcp.servers.*.env`** are not overwritten by empty lines in the repo root `.env`.
- **Auto-create file location:** `~/.openclaw/lila-payer.secret` (Windows: `%USERPROFILE%\.openclaw\lila-payer.secret`). Check **gateway stderr** for `[lila-mcp] Auto-created payer wallet` or `[lila-mcp] Payer after bootstrap:`.
- **`lila_payer_status` not in the tool list:** use **`lila_services`** and read **`mcpClient.payerStatus`** (same payload). Restart the gateway after `git pull` so the new tool registers.
- **`trustline entry is missing for account`:** the payer account must hold **testnet USDC** for x402. Order: (1) send **XLM** to the payer **G** address (testnet faucet), (2) add a **USDC trustline** for issuer `GBBD47IF6LWK7P7MDEVSCWR7DPUWV3NY3DTQEVFL4NAT4AQH3ZLLFLA5`, (3) get **USDC** (e.g. [Circle testnet faucet](https://faucet.circle.com) → Stellar Testnet). From repo root: `LILA_PAYER_SECRET=S... npm run payer:trustline` (adds the trustline; does not fund).
